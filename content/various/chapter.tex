...
\section{Optimization tricks}
	...
	\subsection{Bit hacks}
      ...
      \item \verb@for (int x = m; ; x = (x - 1) & m)@ \\ \verb@  // Here x is a submask of m@ \\ \verb@  if (!x) break;@ 
			\item \verb@c = x&-x, r = x+c;@ \\ Now \verb@ (((r^x) >> 2)/c) | r@ is the next number after \texttt{x} with the same number of bits set.
			\item \verb@rep(b,0,K) rep(i,0,(1 << K))@ \\ \verb@  if (i & 1 << b)@ \\ \verb@    D[i] += D[i^(1 << b)];@ \\computes all sums of subsets, aka SOS DP.
		\end{itemize}

    \kactlimport{variableBitset.h}
